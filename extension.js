...
let outputChannel = vscode.window.createOutputChannel('chatgpt-note-helper');;
...
let maxToken = Number(vscode.workspace.getConfiguration().get('chatgpt-note-helper.maxToken'));
if (!maxToken)
	maxToken = Number(3000);

let temperature = Number(vscode.workspace.getConfiguration().get('chatgpt-note-helper.temperature'));
if (!temperature)
	temperature = 0.1;

let isLongConversationMode = Boolean(Number(vscode.workspace.getConfiguration().get('chatgpt-note-helper.isLongConversationMode')));
...
const ask = vscode.commands.registerCommand("chatgpt-note-helper.ask", () => {
	...
	fetch(url, {
		...
		.then(data => {
			if (data.error) {
				//console.log(data);
				printChannelOutput(`Error: ${data.error.message}`);
